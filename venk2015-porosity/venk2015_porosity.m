...
sing_val=30; %no singular values
...
%% Step 1: ILT (use BRD)
results_leng = 10;
bins_ILTold = zeros(Ny,results_leng);
...
for i = 1:results_leng
    ...
    f_calibrate, K2, N2, Ny, tE, T2, tau2, porosity); 
       
    bins_ILTold(:,1) = f_est_ilt;


    figure(50)
    clf
    hold on
    plot(T2, f_calibrate,'-b');
    plot(T2, f_est_ilt,'-r');
...
%bias_T2 = [bias_T2(1:10)  ones(1,20)];

r_t2 = (bias_T2 +1)./n_std_dev;
correction_T2 = 1./(1 + bias_T2 .* (r_t2 ./ (sum(r_t2) + r_t2)    ));
