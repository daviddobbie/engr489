...
% Bayesian NMR Relaxometry, paper 6 - Paul Teal
% 
% 
...
for indx = 1:tot_Prior
    ...
    clf
    hold on
    plot(T2,f_mean_prior_interpolated, '--' , 'LineWidth',2)
    plot(T2, mean(interpol_prior))
    hold off
    set(gca, 'XScale', 'log')  
    %}
    ...
    priorNormalityTest = normalityTest(interpol_prior');
    
    log_interpol_prior = interpol_prior;
    
    %log_interpol_prior(~log_interpol_prior) = []; % remove zero values
    
    priorLogNormalityTest = normalityTest(log(interpol_prior)'); 
    ...
    lgnd = legend('f left out ','f prior mean \mu_f')
    ...
    subplot(2,1,2)
    hold on
    plot(T2, priorNormalityTest)
    plot(T2, priorLogNormalityTest)    
    plot([min(T2) max(T2)], [0.05 0.05], '--')
    hold off
    ylabel('KS Test Result')
    xlabel('$T_2$')    
    lgnd = legend('Standard Normal Noise','Log Normal Noise')
    set(gca, 'XScale', 'log')   
...
% Takes the matrix of prior data results and test how Gaussian the prior
% results are for the experiment. 
% INPUTS: 
%    prior: matrix of prior results
% OUTPUTS:
%    results: vector of results from the kstest (for normality)
function [results] = normalityTest(prior)
    [density_fn_length number_of_samples] = size(prior)
    results = zeros(density_fn_length,1);
    
    for idx = 1:density_fn_length
        ...
        %normalise vect for normal dist     
        if(sigma > 0)
           data_test_vector = (priorData - mu)/sigma;
        else
           data_test_vector = (priorData - mu)/1; %safe division
        end
 ...
 % remove zero values
        ...
        %{
        vect_log = log(priorData);
        mu_log = mean(priorData);
        sigma_log = std(priorData);
        
        if(sigma_log > 0)
            vect_log = (priorData - mu_log)/sigma_log;
            figure(74)
            hold on
            cdfplot(vect_log)
            hold off
        ...
        % returns: H_result =1, p< 0.05 if the data vector is NOT a from a
        % standard normal distribution.
        if (norm(data_test_vector) > 0) %makes sure we don't have a full zero vector
            [h_result  results(idx)] = kstest(data_test_vector);
        else
            results(idx) = 0;
        end
