...
function [estimate, compute_time ,f_est, estimate_bfv] = iltx_estimator(g_bfv, m, K, n_sigma,...
    ...
    %momentKern(:,15:30) = 0.02; % remove for long relaxation
    
    % weighting measurements with short pulse increased SNR
    
    
    weight_short_pulse = ones(size(m));
    weight_short_pulse(1:30) = 11* ones(30,1);
...
%{
   ...
   %iltx_inversion_multiple(G_opt, L_opt, W_opt, n_sigma_avg, T2)
    ...
    estimate = estimate_bfv / porosity;
