...
taperedArea = actualTaperedArea(f_answer, Tc, T2)
hold on
p = plot([taperedArea taperedArea], [0 max(h1.Values)]);
p.LineWidth = 3;
p.Color = 'k';
...
p = plot([taperedAreaMean_mean taperedAreaMean_mean], [0 max(h1.Values)]);
p.LineWidth = 2.5;
p.Color = 'm';

legend('Frequency of Tapered Areas','Actual Tapered Area','Estimated Tapered Area')
